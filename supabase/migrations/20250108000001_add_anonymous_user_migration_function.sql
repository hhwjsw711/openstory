-- Migration: Add transactional anonymous user data migration function
-- Description: Ensures atomic data transfer during anonymous-to-authenticated account linking
-- Prevents data loss from race conditions and partial failures

-- Function to migrate all anonymous user data to authenticated user in a single transaction
...
-- Grant execute permission to authenticated users (BetterAuth uses service role)
GRANT EXECUTE ON FUNCTION migrate_anonymous_user_data(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION migrate_anonymous_user_data(uuid, uuid) TO service_role;
