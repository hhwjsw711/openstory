 ...
 * POST /api/sequences/$sequenceId/frames/$frameId/generate-variants - Generate variant image for a frame
 */
    ...
    handlers: {
      POST: async ({ request, params }) => {
        try {
            ...
            return json(
              {
                success: false,
                message:
                  'Frame must have a thumbnail image to generate variants',
                timestamp: new Date().toISOString(),
              },
              { status: 400 }
            );
          ...
          console.error(
            '[POST /api/sequences/$sequenceId/frames/$frameId/generate-variants] Error:',
            error
          );
