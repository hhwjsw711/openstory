...
export const MotionModelSelector: React.FC<MotionModelSelectorProps> = ({
  ...
  const availableModels = useMemo(() => {
    return Object.entries(IMAGE_TO_VIDEO_MODELS).map(([key, model]) => ({
      ...model,
      id: key as ImageToVideoModel,
    }));
  }, []);
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-[11px] text-muted-foreground">
                    Max {model.capabilities.maxDuration}s
                    {model.capabilities.supportsPrompt && ' • Prompt support'}
                  </span>
                </div>
              </DropdownMenuCheckboxItem>
            );
